...
- [X] T058 The Durable Object never writes its status back to D1, so `draft_sessions.status` stays `armed` for the whole draft. Confirmed in production after a completed mock draft: both rows still read `armed`, `completed_at` NULL. Two consequences — the diagnostic surface reports a stale status (the snapshot is correct, which is why it looked fine), and `sweepAction`'s `armed` deadline can therefore fire against a session that is actually live, since nothing ever writes `live`. The `live`/`armed` distinction exists precisely to prevent that and is currently unreachable
- [X] T048 **PARTIAL — the comparison is built and proven; nothing calls it.** `src/draft/oracle.ts` exists and agrees with ESPN's record 72/72 on the real corpus, but NO code path invokes it on completion. Production confirms it: after a full mock draft, `draft_archives` and `draft_picks` are both empty. It was marked done on the strength of the module and its tests rather than the wiring. **To close:** trigger it on completion — and note the design correction found while checking: the ESPN fetch must NOT happen inside the Durable Object, because FR-024a forbids the session holding a credential. It belongs in the cron sweep, which already loads credentials. Original: Implement `src/draft/archive.ts`: on `drafted`, fetch the authoritative post-completion `mDraftDetail` and **reconcile the tap-built draft against it before archiving**. 010 used this oracle in tests, where it earned its keep twice — disproving the field-3 reading (5/70) and confirming the ledger offsets (31/31). Self-consistency cannot catch a systematically missed pick; an independent source can
- [X] T049 **PARTIAL — `reconciledPicks` exists and is mutation-tested, but is never called.** Same false marking as T048. Original: Bump the revision through the existing correction path on divergence in `src/draft/archive.ts`, and record the divergence rather than silently preferring one source
- [X] T050 **PARTIAL — `writeArchive` exists in `src/db/draft.ts` with chunking and first-seen-wins, but has zero call sites.** Verified against production: 0 archives after a completed mock draft. Original: [P] Write the archive in chunked batches, first-seen-wins, in `src/db/draft.ts`; retained **indefinitely** as season history (ratified 2026-08-02)

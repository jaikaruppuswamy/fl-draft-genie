...
- [ ] T056 [US4] Relay the ledger's `totalSlots` alongside its filled picks. 005 currently derives the draft's length as `team_count × roster spots`, which is right for a standard league but errs HIGH for keepers — and a total that is never reached means a draft never completes or archives. ESPN states the real figure in the INIT ledger and `tap/decode.ts` already reads it as `totalSlots`; it is simply not relayed. Numeric, carries no identity, so FR-006a is unaffected
...
- [X] T028 [P] [US2] Replay test in `tests/tap/replay.test.ts`: the T008 capture through decode → filter → batch produces the pick sequence in the T009 oracle, compared against the **independent** oracle rather than against itself

### Implementation for User Story 2

- [X] T029 [US2] Implement `tap/intercept.ts`: `new Proxy(Native, { construct })` for **both** `WebSocket` and `EventSource` — ESPN is WebSocket-first with SSE fallback reached in ~7 s of bad network, so a WebSocket-only tap goes dark exactly when the network is worst. `Reflect.construct(target, args, newTarget)`, URL-scoped by hostname, everything in `try/catch` (a throw anywhere in the trap propagates to the page's `new`), observation attached via a saved `addEventListener` reference, decode deferred with `setTimeout(…, 0)` — **not** `queueMicrotask`, which still drains on ESPN's critical path (depends on T016, T018, T020)
- [X] T030 [US2] Implement `src/api/tap.ts` with `POST /api/tap/batch`, `POST /api/tap/status`, `GET /api/tap/health`, plus CORS preflight handling. Keep hono's CORS behaviour of **omitting** rather than rejecting on an unlisted origin — that is what the `GM_xmlhttpRequest` path needs; do not "fix" it with a 403 guard
- [X] T031 [US2] Mount the tap routes in `src/api/app.ts` **before** `app.use("/api/*", …)` at [app.ts:34](src/api/app.ts:34) — that middleware is a bare prefix match, so a tap POST reaching it first returns 401 regardless of how correct the token is (depends on T030)
- [X] T032 [US2] Implement `src/db/tap.ts`: issue, verify (by token hash), bind `install_id` on first use, touch `last_used_at`, revoke, rotate. Verification scopes each message's league to a connection the account owns — the credential is per-user, the league is per-message, which is how 005 FR-007d's per-connection scoping is satisfied (depends on T014)
- [X] T033 [US2] Implement `tap/main.ts`, the impure shell (~50 lines): install the wrapper at document-start, run the **page-world preflight**, wire the pure modules, and relay via `GM_xmlhttpRequest` with `anonymous: true`. **No function the page can reach may close over the pairing token** — under `@sandbox raw` the tap shares ESPN's JS realm (depends on T029, T024, T022)
- [X] T034 [US2] Make the flush **event-driven** in `tap/main.ts` — next observed message, `online`, `visibilitychange`, `pageshow`, `resume` — with a timer only as a backstop. A chained `setTimeout` in a hidden tab is throttled to one per second and then **one per minute**, which alone would fail SC-005's 60-second recovery (depends on T033)
...
- [X] T041 [P] [US4] Preflight test in `tests/tap/preflight.test.ts`: the page-world assertion fails when the wrapped global is not the page's. In an isolated world `window.WebSocket` is not ESPN's and the tap would observe nothing **while appearing healthy** — Tampermonkey documents that `@sandbox raw` can fall back to another sandbox under CSP, so this is a real, silent failure mode. **Rewritten:** the original test simulated the isolated world as a scope with no transport global at all, which is a different failure entirely and is why it passed against a broken preflight. It now builds a genuine second realm with `node:vm`, so the wrapped-but-wrong-global case is exercised for real

### Implementation for User Story 4

- [X] T042 [US4] **LIVE-VALIDATED (2026-08-05).** The shipped tap 0.1.7 attached under desktop Chrome and relayed a full 72-pick mock draft (72 batches / 75 messages in 7m 46s), so `provePageWorld` passes in the real environment against the real ESPN page. The FAILURE path — driving `INCOMPATIBLE` when the preflight does not pass — remains unobserved live and is covered only by the `node:vm` cross-realm unit tests; triggering it deliberately would require forcing Tampermonkey out of page context. Implement the page-world preflight in `tap/main.ts`: assert **membership of the page realm**, not merely that the global is non-native, and drive `INCOMPATIBLE` loudly on failure (depends on T033, T025). The first version was marked done while returning `wrapped.length > 0`, which is true in the isolated world it exists to catch — it asserted only that *a* global existed and was replaced. Now `provePageWorld` (`tap/intercept.ts`) requires an independent page handle (`unsafeWindow`), that we installed on *that* object, that our marker reads back off it, and that a page global distinct from `window` is genuinely cross-realm; anything unproven reports `INCOMPATIBLE` with a reason rather than assuming health. Tested against a real second realm via `node:vm`, and the tests were confirmed to fail against the old implementation. **To close:** confirm in a real draft room that the badge reads a healthy state, and that forcing Tampermonkey out of page context produces `INCOMPATIBLE`.
...
- [ ] T045 [US4] **STILL PARTIAL — and the mock draft could NOT have validated it.** The session derived `totalPicks = 0` at the time (a 005 defect, since fixed), so completion was unreachable and draft-end could not fire. Production confirms it: `completed_at` NULL after a draft that demonstrably finished. Re-check on the next full draft, now that the total is derived from the roster shape and the status is mirrored to D1. Implement draft-end detection (FR-024): stop relaying and say so; where the tap cannot tell whether a draft is running, report that uncertainty rather than going quiet. The first version was marked done while doing neither: it only re-rendered a badge when an INIT ledger arrived already full, so it never stopped relaying, and in a single-session draft no INIT arrives after the last pick — it could not fire at all. Logic now lives in `tap/draftEnd.ts` (a pure module, because its previous home in the shell is what made it untestable): completion counts the **union** of incremental picks and the ledger — the observed draft delivered 69 of 72 by stream and 3 only by ledger, so either source alone stalls forever — the denominator is taken only from a ledger and never guessed, picks and ledgers stop while status keeps flowing, and a long silence without confirmed completion reports the new `draft-end-unknown` state. Mutation-tested against both shipped behaviours. **To close:** observe a real draft reaching completion and confirm the relay stops and the state is reported.
...
- [X] T053 **LIVE-VALIDATED (2026-08-05).** A full mock draft with the shipped tap running in one manager's Chrome and Draft Genie open on a SECOND device (Safari) — the cross-device split the design ratified. Owner reported the board tracked seamlessly. 72 picks relayed, all applied. End-to-end validation (SC-001, SC-011): a real draft with the shipped tap, Draft Genie open on a second device, confirming 100% of picks relayed against ESPN's post-draft record — and, if a second league drafts, that both relay independently from one install
...
- [X] T055 [US4] Implement periodic liveness in `tap/heartbeat.ts` (pure) + `tap/main.ts` and accept it at `POST /api/tap/status` (FR-015a; 005 FR-007e). Added because 005's round-4 clarify established that a tap reporting only on state *change* is silent precisely when healthy, and pick silence cannot substitute — measured gaps run ~1 s (autodraft) to 90 s+ (human). Carries a `hidden` flag: a background tab's timers throttle to ~1/minute, so a single lapse threshold would mark a healthy backgrounded tap dead, and the ratified design *expects* that tab to be backgrounded. Also heartbeats on the wake events, including the transition into hidden, so the receiver widens its bound before throttling starts. The status route additionally gained the schema and identifier/URL boundary check it never had — it previously logged whatever arrived, scrubbed only by the tap. Mutation-tested: routing the heartbeat back through the change-gate fails the suite. Tap 0.1.6

...
sync, and the pre-draft auto-sync cron. 68 tests green. Sign-in emails are
delivered via Cloudflare Email Service (`send_email` binding, domain
`neelamjai.com` onboarded — no API keys). Remaining before 001 closes: live
validation against real ESPN leagues (task T045, quickstart scenarios).
